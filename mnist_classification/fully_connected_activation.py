...
def do_eval(sess,
    ...
    if simple_run:
        if steps_per_epoch > 1000 // local_batch_size:
            steps_per_epoch = 1000 // local_batch_size
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        total_steps = 100000 if need_validation else 60000
        total_steps = np.maximum(int(total_steps * np.sqrt(training_samples / 60000.0)), 10000)
        ...
        for epoch in xrange(total_epoches):
            ...
            for step in xrange(batch_per_epoch):
                ...
                if FLAGS.feedback:
                    ...
                    unlearn_rate = np.minimum(np.maximum((try_value + 0.2 - true_value) * 5.0, 0.00), 1.0) \
                            if training_samples > 1000 else np.minimum(np.maximum((try_value + 0.5 - true_value) * 2.0, 0.00), 1.0)
                    ...
                    # There is a problem: if there are only one non-zero term in unlearn rate, it will be too big
            ...
            if epoch % (total_epoches // 40) == 0:
                if need_validation:
                    validation_accuracy = do_eval(sess, activation, images_placeholder, input_labels_placeholder,
                                                  labels_placeholder, data_sets.validation, False)
                mean_activation = do_activation_value(sess, activation, images_placeholder,
                                                      input_labels_placeholder,
                                                      labels_placeholder, data_sets.train, True)
                if need_validation and validation_accuracy > best_validation_accuracy:
                    best_validation_accuracy = validation_accuracy
                    best_train_accuracy = do_eval(sess, activation, images_placeholder,
                                                  input_labels_placeholder,
                                                  labels_placeholder, data_sets.train, False)
                    best_test_accuracy = do_eval(sess, activation, images_placeholder, input_labels_placeholder,
                                                 labels_placeholder, data_sets.test, False)
                    checkpoint_file = os.path.join(FLAGS.log_dir, 'model.ckpt')
                    saver.save(sess, checkpoint_file, global_step=epoch)
                    print("%.04f, %.04f, %.04f, %.04f" % (
                        epoch + 1, best_train_accuracy, best_test_accuracy, mean_activation))
                else:
                    ...
                    print("%.04f, %.04f, %.04f, %.04f" % (
                        epoch + 1, train_accuracy, test_accuracy, mean_activation))
        ...
        if not need_validation:
            ...
            best_test_accuracy = do_eval(sess, activation, images_placeholder, input_labels_placeholder,
                                         labels_placeholder, data_sets.test, False)
            checkpoint_file = os.path.join(FLAGS.log_dir, 'model.ckpt')
            saver.save(sess, checkpoint_file, global_step=total_epoches)
