...
def print_images(given_images, infer_images, filename):
    given_images = np.divide(given_images, np.maximum(np.max(given_images, axis=1, keepdims=True), 1e-5))
    infer_images = np.divide(infer_images, np.maximum(np.max(infer_images, axis=1, keepdims=True), 1e-5))
    images = np.concatenate([given_images, infer_images], axis=1)
    # images = np.multiply(images, np.greater(images, 0.5))
...
def print_init_images(images, filename):
...
def run_inference():
    ...
    with tf.Graph().as_default():
        ...
        masked_images = mask_images(images_feed, 14, False)
        print_init_images(masked_images, "complete_init")
        top_image = images_feed[:, : mnist.IMAGE_PIXELS // 2]
        given_image = tf.divide(tf.constant(top_image, tf.float32), 10.0)

        # init_image = tf.random.uniform([FLAGS.batch_size, mnist.IMAGE_PIXELS // 2],
        #                                minval=0.0,
        #                                maxval=0.005,
        #                                dtype=tf.float32)
        init_image = tf.zeros([FLAGS.batch_size, mnist.IMAGE_PIXELS // 2], tf.float32)
        init_label = tf.zeros([FLAGS.batch_size, mnist.LABEL_PIXELS], tf.float32)
        image_variable = tf.Variable(init_image, name="image_variable")
        label_variable = tf.Variable(init_label, name="label_variable")
        image = tf.concat([given_image, image_variable], axis=1)
        # image = tf.minimum(image, 0.1)
        activation = inference(image, label_variable, FLAGS.feature_layers, tf.ones([FLAGS.batch_size], tf.float32))

        global_step = tf.Variable(0, name='global_step', trainable=False)
        # Gradient
        over_term = tf.nn.relu(image_variable - 1.0) + tf.nn.relu(-image_variable)
        norm_image = tf.divide(image_variable, tf.sqrt(
            tf.reduce_sum(tf.multiply(image, image), axis=1, keepdims=True)))
        target_function = tf.reduce_sum(tf.multiply(over_term, over_term), axis=1) \
                          + tf.reduce_sum(tf.abs(norm_image), axis=1) * 0.003 \
                          - tf.pow(activation, 1.0)

        #  + tf.reduce_sum(tf.abs(norm_image), axis=1) * 0.003 \
        # target_function = tf.reduce_sum(tf.multiply(over_term, over_term), axis=1) - tf.pow(activation, 1.0)
        ...
        # norm_op = tf.assign(image_variable, tf.maximum(tf.minimum(image_variable, 0.1), 0.0))
        ...
        groups = []
        for i in range(10):
            labels = tf.multiply(tf.ones([FLAGS.batch_size], tf.int32), i)
            ...
            sess.run([tf.assign(label_variable, labels)])
            groups.append(sess.run(activation))
        labels = tf.constant(np.argmax(np.array(groups), axis=0), tf.int32)
        labels = tf.one_hot(labels, depth=NUM_CLASSES, on_value=1.0, off_value=0.0, axis=-1)
        labels = tf.divide(labels, tf.sqrt(tf.constant(PIXEL_PER_CLASS, tf.float32)))
        labels = tf.tile(labels, [1, PIXEL_PER_CLASS])
        sess.run([tf.assign(label_variable, labels)])

        image_file_name = "complete_images"
        for i in range(10000000):
            ...
            if i % 10000 == 0:
                ...
                if i % 100000 == 0:
                    print_images(top_image, sess.run(image_variable), image_file_name)
            sess.run([train_op])
            # sess.run([norm_op])
        print("end")
        print_images(top_image, sess.run(image_variable), image_file_name)
