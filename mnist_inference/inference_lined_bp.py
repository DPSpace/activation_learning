...
def do_gd_eval(sess,
    ...
    op1 = tf.assign(label_variable, tf.zeros([FLAGS.batch_size, mnist.LABEL_PIXELS], tf.float32))
    for step in xrange(steps_per_epoch):
        images_feed, labels_feed = data_set.next_batch(FLAGS.batch_size,
                                                       FLAGS.fake_data)

        # init_value = np.random.uniform(0.0, 0.1, [FLAGS.batch_size, mnist.LABEL_PIXELS])
        # op1 = tf.assign(label_variable, tf.constant(init_value, tf.float32))
        sess.run([op1])
        feed_dict = {
        ...
        init_point = sess.run(label_variable, feed_dict=feed_dict)
        outputs = np.multiply(init_point, init_point)
        outputs = np.sum(np.reshape(outputs, [FLAGS.batch_size, mnist.PIXEL_PER_CLASS, mnist.NUM_CLASSES]), axis=1)
        result = np.argmax(outputs, axis=1)
        # outputs = np.reshape(init_point, [FLAGS.batch_size, mnist.PIXEL_PER_CLASS, mnist.NUM_CLASSES])
        # outputs2 = outputs - 1.0 / np.sqrt(mnist.NUM_CLASSES)
        # outputs = np.sum(np.multiply(outputs2, outputs2) - np.multiply(outputs, outputs), axis=1)
        # result = np.argmin(outputs, axis=1)

        # for i in range(5):
        #     if result[i] != labels_feed[i]:
        #        print(outputs[i], result[i], labels_feed[i])
        count_in_batch = np.sum(np.equal(result, labels_feed))
        print(step, count_in_batch)
        true_count += count_in_batch
...
def run_inference():
    ...
    with tf.Graph().as_default():
        ...
        # Build a Graph that computes predictions from the inference model.
        train_outputs = mnist.inference(images_placeholder, FLAGS.feature_layers, False)
        ...
        # test_accuracy = do_gd_eval(sess, train_op, label_variable, images_placeholder,
        #                           labels_placeholder, data_sets.test)
        # train_accuracy = do_gd_eval(sess, train_op, label_variable, images_placeholder,
...
def run_experiments6():
    path = os.path.join("data", "experiments6_bp.txt")
